...
def graph_analysis1():
	...
	if user_attr1=='':
		...
		table_head = " "+schema1[int(schema_attr1[1:])-1]+" "

		graph_data = line_chart.render_data_uri()
		
		#Rendering the display1.html file and sending the arrays and graph dat to the browser , i.e..,display1.html page
		return render_template("index3.html",display1 = display1 , schema1 = schema1 , display_count = display_count , graph_data=graph_data, table_head=table_head)
	...
	#If user_attr1 isn't empty
		...
		line_chart = pygal.Bar()
		line_chart.title = 'Graph analysis for '+schema1[int(schema_attr1[1:])-1]
		print 'Graph analysis for '+schema1[int(schema_attr1[1:])-1]
		for i in range(1,len(display1)):		
			line_chart.add(display1[i], int(display_count[i]))
...
def graph_analysis2():
	...
	ldisplay = []
	check = []
	target = open("static/temp_files/check_temp.txt","r+")
	repeat = target.read()
	check = repeat.split(',')
	target.close()
	...
	empty_array1 = pd.read_csv("static/temp_files/temp_data2.csv",delimiter = ",")
	x_labels = empty_array1.as_matrix(columns = None)
	
	row_len = len(empty_array.index)
	col_len = len(empty_array.columns)
	labels_len = len(empty_array1.index)
	print "\n------------",empty_array
	print "\n-------------",empty_array_display
	arr = []
	table_header = []
	table_head = user_attr2
	if user_attr2 != '':
		table_head = user_attr2
	else:
		for j in range(labels_len):
			table_header.append(x_labels[j][1]) 
	print "this is within if loop"
	len1 = len(empty_array)
	line_chart = pygal.Bar()

	if check[0] == "compare":
		print "if both are empty"
		for i in range(row_len):
			print empty_array_display[i][1]
			print empty_array_display[i][2:]
			line_chart.add(empty_array_display[i][1],empty_array_display[i][2:])
	...
	for j in range(labels_len):
		arr.append(x_labels[j][1])
	print arr
	line_chart.xlabels = map(str,arr)
	graph_data = line_chart.render_data_uri()
	
	print table_header

	return render_template("index4.html" ,row_len = row_len,col_len=col_len, empty_array_display = empty_array_display , empty_array = empty_array,graph_data=graph_data , table_head = table_head)
